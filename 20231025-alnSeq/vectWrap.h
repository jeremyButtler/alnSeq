...
^    o memBlockST:
^      - Pointer to a vectMemBlock structure to modify/free
^    o size:
^      - is size of buffer to assigne the vectMemBlock
\<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
{ /*mallocVectMemBlock*/ \
   ...
   (memBlockST)->freePtr = \
      malloc(sizeof(int8_t) * ((size) + defVectBytes -1));\
   \
   if((memBlockST)->freePtr != 0)\
   { /*If: I had no problems*/\
      /*Set up pointer to aligned memory*/\
      (memBlockST)->alnPtr =\
         (int8_t *)(\
             ((uintptr_t) (memBlockST)->freePtr)\
...
{ /*callocVectMemBlock*/ \
   ...
   /*Allocate unaligned memory*/\
   (memBlockST)->freePtr = \
      calloc(((size) + defVectBytes - 1), sizeof(int8_t));\
   \
   if((memBlockST)->freePtr != 0)\
   { /*If: I had no problems*/\
      /*Set up pointer to aligned memory*/\
